 ...
 * module per case, and add/remove evidence items (with optional file
 * upload and a "contradictory by design" flag used for the
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <form onSubmit={handleCreateCase} className="space-y-3">
          <div>
            <Label>Title</Label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="e.g. Mock Burglary Scene A" />
          </div>
          <div>
          ...
          <label className="flex items-center gap-2 text-sm text-slate-700">
            <input type="checkbox" checked={bayesianEnabled} onChange={(e) => setBayesianEnabled(e.target.checked)} />
          ...
          <Card key={c.id}>
            <div className="flex items-start justify-between">
              ...
              <div className="flex gap-2 text-xs">
                <span className={`px-2 py-1 rounded ${c.is_active ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
                  {c.is_active ? "Active" : "Inactive"}
                </span>
                <span className={`px-2 py-1 rounded ${c.bayesian_enabled ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-500"}`}>
            ...
            <div className="flex gap-2 mt-3">
              <Button variant="secondary" onClick={() => toggleBayesian(c.id, c.bayesian_enabled)}>
                Toggle Bayesian
              </Button>
              <Button variant="secondary" onClick={() => toggleActive(c.id, !!c.is_active)}>
            ...
            {expandedCaseId === c.id && (
              <EvidenceManager caseDetail={c} onChanged={() => loadCaseDetail(c.id)} />
            )}
          </Card>
        ))}
      </div>
...
function EvidenceManager({ caseDetail, onChanged }: { caseDetail: CaseDetail; onChanged: () => void }) {
    ...
    <div className="mt-4 border-t border-slate-100 pt-4">
      ...
      <ul className="space-y-2 mb-4">
        {caseDetail.evidence_items?.map((item) => (
          <li key={item.id} className="flex items-start justify-between bg-slate-50 rounded-md p-2">
            <div>
              <p className="text-sm font-medium text-slate-800">
                {item.label}
                {item.is_contradictory_by_design && (
                  <span className="ml-2 text-xs text-red-500 bg-red-50 px-1.5 py-0.5 rounded">contradictory by design</span>
                )}
              </p>
              <p className="text-xs text-slate-500">{item.description}</p>
              {item.file_path && <p className="text-xs text-slate-400 mt-1">File: {item.file_path}</p>}
            </div>
            <Button variant="ghost" onClick={() => handleDelete(item.id)} className="text-red-500 text-xs">
              Remove
            </Button>
          </li>
        ))}
      ...
      <form onSubmit={handleAdd} className="space-y-2 bg-white border border-slate-100 rounded-md p-3">
        <div>
          <Label>Label</Label>
          <Input value={label} onChange={(e) => setLabel(e.target.value)} required placeholder="e.g. Item 4: Blood spatter pattern" />
        </div>
        <div>
        ...
        <label className="flex items-center gap-2 text-sm text-slate-700">
          <input type="checkbox" checked={isContradictory} onChange={(e) => setIsContradictory(e.target.checked)} />
